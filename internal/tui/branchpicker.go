...
func newBranchPicker(th Theme, branches []string, width, height int) branchPicker {
	q := textinput.New()
	q.Prompt = "> "
	...
	p := branchPicker{th: th, query: q, all: branches, width: width, height: height}
	p.filter()
	return p
}
...
func (p branchPicker) view() string {
	...
	b.WriteString(p.th.titleStyle.Render("Pick branch"))
	b.WriteString("\n")
	...
	case len(p.all) == 0 && len(p.matches) == 0:
		b.WriteString(p.th.dimStyle.Render("  no branches — type to create one"))
	case len(p.matches) == 0:
		b.WriteString(p.th.dimStyle.Render("  no match — ↵ creates this branch"))
	default:
	...
	b.WriteString("\n\n")
	b.WriteString(p.th.dimStyle.Render(p.footer()))
	return p.th.pickerStyle.Render(b.String())
}
...
func (p branchPicker) row(m branchMatch, current bool, w int) string {
	if current {
		return p.th.selRowTitleStyle.Render(pad("▌ "+m.name, w))
	}
	line := "  " + highlight(p.th, m.name, m.positions)

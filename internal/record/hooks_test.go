...
func TestCopilotHookInstall(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("hook not at %s: %v", hookPath, err)
	}
	got := string(data)
	...
	if !strings.Contains(string(excl), ".github/hooks/wasa.json") {
		t.Errorf("exclude entry missing:\n%s", excl)
	}
	...
	if got := InstalledAgents(repo); len(got) != 0 {
		t.Errorf("agents still installed after remove: %v", got)
	}
...
// TestCopilotHookLeavesForeignFile checks a non-wasa file at the copilot repo
// hook path is neither overwritten by install nor deleted by remove.
func TestCopilotHookLeavesForeignFile(t *testing.T) {
	repo := initRepo(t)
	path := filepath.Join(repo, ".github", "hooks", "wasa.json")
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil {
		t.Fatal(err)
	}
	foreign := `{"version":1,"hooks":{"sessionEnd":` +

...
func TestSearch(t *testing.T) {
	...
	mustWrite(t, dir, Checkpoint{
		Meta:   Meta{SessionID: "sess1", Branch: "task/retry", WasaVersion: "t"},
	...
	mustWrite(t, dir, Checkpoint{
		Meta:       Meta{SessionID: "sess2", Branch: "task/cache", WasaVersion: "t"},
		Intent:     "tidy up config",
		Transcript: []byte(`{"role":"assistant","text":"let us drop the cache here"}`),
	})
	...
	if hits[0].Meta.SessionID != "sess1" || hits[0].File != "intent" {
		t.Errorf("hit = %s/%s, want sess1/intent", hits[0].Meta.SessionID, hits[0].File)
	}
	if hits[0].LineText != "rework the retry logic with backoff" {
		t.Errorf("snippet from wrong (not newest) checkpoint: %q", hits[0].LineText)
	}
	...
	if len(trans) != 1 || trans[0].Meta.SessionID != "sess2" || trans[0].File != "transcript" {
		t.Fatalf("transcript search = %+v, want sess2/transcript", trans)
	}

	if h, _ := Search(dir, SearchOpts{Query: "drop the cache", IntentOnly: true}); len(h) != 0 {
		t.Errorf("intent-only matched transcript phrase (%d hits)", len(h))
	}

	if h, _ := Search(dir, SearchOpts{Query: "e", Branch: "task/retry"}); len(h) != 1 || h[0].Meta.SessionID != "sess1" {
	...
	if h, _ := Search(dir, SearchOpts{Query: "retry.*backoff", Regex: true}); len(h) != 1 {
	...
	if h, _ := Search(dir, SearchOpts{Query: "retry logic", Since: past}); len(h) != 1 {
		t.Errorf("since=past matched %d, want 1", len(h))
	}

	if h, err := Search(dir, SearchOpts{Query: "no-such-string-xyz"}); err != nil || len(h) != 0 {
...
func TestSearchWithoutRef(t *testing.T) {
	dir := initRepo(t)
	if _, err := Search(dir, SearchOpts{Query: "anything"}); !errors.Is(err, ErrNoRecord) {
